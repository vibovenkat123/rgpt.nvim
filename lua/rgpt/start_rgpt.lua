...
function M.start_reviewing(config)
    ...
    local command = string.format("rgpt --input \"$(git diff %s)\" --model \"%s\" --max %s --temp %s --topp %s --freq %s --pres %s --best %s", current_path, model, max_tokens, temp, top_p, frequence_pen, presence_pen, best_of)
